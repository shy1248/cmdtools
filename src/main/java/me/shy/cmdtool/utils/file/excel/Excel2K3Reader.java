 ...
 * @fileName: com.ysb.common.utils.file.excel.Excel2K3Reader.java
 * @version: 1.0
 * @since: 2015-3-29 上午9:38:32
 * @describe : 抽象Excel2003读取器，通过实现HSSFListener监听器，采用事件驱动模式解析excel2003
 *           中的内容，遇到特定事件才会触发，大大减少了内存的使用。
 * 
 *           ALL RIGHTS RESERVED,COPYRIGHT(C) Yu Shuibo, LIMITED 2015.
 */
public class Excel2K3Reader implements HSSFListener {
    ...
    public void processRecord(Record record) {
        ...
        case LabelRecord.sid:
            ...
            this.cells.add(thisColumn, value);
            break;
        case LabelSSTRecord.sid: // 单元格为字符串类型
            ...
            if (sstRecord == null) {
                cells.add(thisColumn, " ");
            } else {
                value = sstRecord.getString(lsrec.getSSTIndex()).toString().trim();
                value = value.equals("") ? " " : value;
                cells.add(thisColumn, value);
            }
            break;
        ...
        if (record instanceof LastCellOfRowDummyRecord) {
            ...
            lastColumnNumber = -1;
            // 每行结束时， 调用getRows() 方法
            rowHandler.getRow(sheetIndex, curRow, cells);
